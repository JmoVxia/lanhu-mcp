    ...
    for idx, L in enumerate(layers):
        ...
        if ltype == 'textLayer' and (L.get('textInfo') or L.get('text')):
            ...
            if ti:
                ...
                # artboard格式处理（MasterGo: text.style.content / Figma: text.value）
                art_style = art_text.get('style') or {}
                text_content = art_text.get('value') or art_style.get('content') or ''
                annot['text'] = text_content
                props.append('z-index:10')
                # 颜色：优先 text.style.color，否则图层 style.fills
                art_color = art_style.get('color') or {}
                color_val = None
                if isinstance(art_color, dict) and art_color.get('value'):
                    color_val = art_color['value']
                if not color_val:
                    fills = ((L.get('style') or {}).get('fills') or [])
                    for fill in fills:
                        if not fill.get('isEnabled', True):
                            continue
                        fc = fill.get('color') or {}
                        if isinstance(fc, dict) and fc.get('value'):
                            color_val = fc['value']
                            break
                if color_val:
...
async def lanhu_get_ai_analyze_design_result(
    ...
    CRITICAL - How to use the returned HTML+CSS (MUST follow this workflow):
        ...
        ⚠️ AUTHORITY PRIORITY (highest → lowest):
            1. lanhu_get_design_structure — authoritative, complete, never-fails source
               of exact properties (see RULE 4). Overlay its values over everything below.
            2. HTML+CSS code  — layout/DOM intent + values for anything not in structure
        ...
        RULE 4 - OVERLAY EXACT PROPERTIES FROM lanhu_get_design_structure (STRONGLY RECOMMENDED):
            The DDS-generated HTML+CSS is often incomplete or inaccurate, and when
            「设计图转代码」is off the DDS schema FAILS entirely (you then only get a
            rough image-based fallback). In BOTH cases you MUST call the tool
            `lanhu_get_design_structure(url, design_name)` and OVERLAY its values.

            lanhu_get_design_structure reads the raw design data directly (never fails)
            and enumerates every property clients need, already in logical points (pt)
            with clean rgb()/rgba() colors:
              size (x,y,width,height) · padding/gaps (relative layout) · color · gradient
              (type/stops/from/to/angle) · border (thickness/color/position/style) ·
              radius (number or per-corner {topLeft,topRight,bottomRight,bottomLeft}) ·
              shadow (color/x/y/blur/spread/inset) · blur · opacity · rotation · blendMode ·
              clip · backgroundImage · text (fontSize/family/weight/color/align/lineHeight/
              letterSpacing/italic/underline/strikethrough).
...
async def lanhu_get_design_structure(
    ...
    """
    从 Sketch/Figma/MasterGo 原始 JSON 提取「结构化图层树」，枚举 iOS/客户端开发所需的全部视觉属性。
    直接读原始设计数据，不依赖 DDS store_schema_revise，未开启「设计图转代码」也一定能用。

    ⭐ 这是设计属性的权威来源（source of truth）。DDS/HTML 经常失败或不准，此工具稳定且完整。
       推荐工作流：无论 DDS 是否成功，都调用本工具，并把这里返回的精确属性「叠加」到最终代码，
    ...
    try:
        ...
        if designs_data.get('status') != 'success':
            return {
                'status': 'error',
                'message': designs_data.get('message', '获取设计图列表失败'),
            }

        designs = designs_data.get('designs') or []
        target = None
        image_id_from_url = params.get('doc_id')
        name_str = str(design_name).strip() if design_name else ''

        if name_str.isdigit():
            index = int(name_str)
            target = next((item for item in designs if item.get('index') == index), None)
        elif name_str:
            target = next((item for item in designs if item.get('name') == name_str), None)
        if target is None and image_id_from_url:
            target = next((item for item in designs if item.get('id') == image_id_from_url), None)

        if target is None:
            return {
                'status': 'need_design_name',
                'message': '未指定设计图。请带上 design_name 或 URL 中的 image_id 后重试。',
